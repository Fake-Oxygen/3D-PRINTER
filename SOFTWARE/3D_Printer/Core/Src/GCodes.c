...
void G0()
{
    ...
    if (E_dif >= 0.0012 || E_dif <= -0.0012)
    {
        ...
        DelayMicrosecond((double)1 / (double)F * (double)60 * MM_PER_REV / STEPS_PER_REV * (double)1000000);
    }
    else
    {
        isRunning = false;
...
void M104()
{
    ...
    {
        SetHeating(HOT_END, -9.0f / 10.0f * (double)Cur_temp + (double)temp_goal + 40.0f);
    }
    else
    {
